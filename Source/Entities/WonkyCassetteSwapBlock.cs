...
{
    [CustomEntity("QuantumMechanics/WonkyCassetteSwapBlock")]
    [TrackedAs(typeof(WonkyCassetteBlock))]
    public class WonkyCassetteSwapBlock : WonkyCassetteBlock
    {
        private class PathRenderer : Entity
        {
            private readonly WonkyCassetteSwapBlock block;
            private Color pathColor;
            private Color pathColorPressed;

            public PathRenderer(WonkyCassetteSwapBlock block)
                : base(block.Position)
            {
                this.block = block;
        ...
        public WonkyCassetteSwapBlock(Vector2 position, EntityID id, int width, int height, Vector2 node, int index, string moveSpec, Color color, string textureDir, int overrideBoostFrames, int controllerIndex, bool noReturn)
        ...
        public WonkyCassetteSwapBlock(EntityData data, Vector2 offset, EntityID id)
            : this(data.Position + offset, id, data.Width, data.Height, data.NodesWithPosition(offset)[1], data.Int("index"), data.Attr("onAtBeats"), data.HexColor("color"), data.Attr("textureDirectory", "objects/cassetteblock").TrimEnd('/'), data.Int("boostFrames", -1), data.Int("controllerIndex", 0), data.Bool("noReturn", false))
        {
        }
