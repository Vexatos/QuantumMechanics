...
{
    [CustomEntity("QuantumMechanics/WonkyCassetteZipMover")]
    [TrackedAs(typeof(WonkyCassetteBlock))]
    public class WonkyCassetteZipMover : WonkyCassetteBlock
    {
        public class PathRenderer : Entity
        {
            ...
            private class Segment
            {
                ...
                public Segment(Vector2 from, Vector2 to)
                {
                    ...
                    Rectangle b = Utilities.Rectangle(from, to);
                    b.Inflate(10, 10);
            ...
            private readonly WonkyCassetteZipMover zipMover;
            ...
            public PathRenderer(WonkyCassetteZipMover zipMover, Vector2[] nodes)
            {
                ...
                for (int i = 0; i < segments.Length; ++i)
                {
                    ...
                    min = Utilities.Min(min, node);
                    max = Utilities.Max(max, node);
        ...
        public WonkyCassetteZipMover(EntityData data, Vector2 offset, EntityID id)
            : this(data.Position + offset, id, data.Width, data.Height, data.NodesWithPosition(offset), data.Int("index"), data.Attr("onAtBeats"), data.HexColor("color"), data.Attr("textureDirectory", "objects/cassetteblock").TrimEnd('/'), data.Int("boostFrames", -1), data.Int("controllerIndex", 0),
        ...
        {
            cog = GFX.Game["objects/QuantumMechanics/wonkyCassetteZipMover/cog"];
            cogPressed = GFX.Game["objects/QuantumMechanics/wonkyCassetteZipMover/cogPressed"];
            cogWhite = GFX.Game["objects/QuantumMechanics/wonkyCassetteZipMover/cogWhite"];
        }
    }
}