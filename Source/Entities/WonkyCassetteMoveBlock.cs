...
namespace Celeste.Mod.QuantumMechanics.Entities
{
    ...
    public class WonkyCassetteMoveBlock : WonkyCassetteBlock
    {
        ...
        public WonkyCassetteMoveBlock(Vector2 position, EntityID id, int width, int height, int index, string moveSpec, Color color, string textureDir, int overrideBoostFrames, int controllerIndex, Directions direction, float moveSpeed)
        ...
        public WonkyCassetteMoveBlock(EntityData data, Vector2 offset, EntityID id)
            : this(data.Position + offset, id, data.Width, data.Height, data.Int("index"), data.Attr("onAtBeats"), data.HexColor("color"), data.Attr("textureDirectory", "objects/cassetteblock").TrimEnd('/'), data.Int("boostFrames", -1), data.Int("controllerIndex", 0), data.Enum("direction", Directions.Left), data.Bool("fast") ? FastMoveSpeed : data.Float("moveSpeed", MoveSpeed))
        {
        ...
        public override void Awake(Scene scene)
        {
            ...
            arrow = new Image(GFX.Game.GetAtlasSubtextures("objects/QuantumMechanics/wonkyCassetteMoveBlock/arrow")[index]);
            arrowPressed = new Image(GFX.Game.GetAtlasSubtextures("objects/QuantumMechanics/wonkyCassetteMoveBlock/arrowPressed")[index]);
            cross = new Image(GFX.Game["objects/QuantumMechanics/x"]);
            crossPressed = new Image(GFX.Game["objects/QuantumMechanics/xPressed"]);
        ...
        private IEnumerator<float?> Controller()
        {
            while (true)
            {
                triggered = false;
                ...
                BreakParticles();
                List<WonkyCassetteMoveBlockDebris> debris = new();
                for (int x = 0; x < Width; x += 8)
                {
                    for (int y = 0; y < Height; y += 8)
                    {
                        ...
                        WonkyCassetteMoveBlockDebris d = Engine.Pooler.Create<WonkyCassetteMoveBlockDebris>().Init(Position + offset, Center, startPosition + offset, spr =>
                        {
                            spr.Color = Activated ? color : pressedColor;
                        });
                        d.Sprite.Texture = Calc.Random.Choose(GFX.Game.GetAtlasSubtextures("objects/QuantumMechanics/wonkyCassetteMoveBlock/debris"));
                        debris.Add(d);
                        Scene.Add(d);
                    }
                }
                ...
                foreach (WonkyCassetteMoveBlockDebris d in debris)
                    d.StopMoving();
        ...
        private IEnumerator<float?> SoundFollowsDebrisCenter(EventInstance instance, List<WonkyCassetteMoveBlockDebris> debris)
        {
            while (true)
            {
        ...
        protected override void HandleUpdateVisualState()
        {
        ...
        private void SetDisabledStaticMoversVisibility(bool visible)
        {
        ...
        private void SetStaticMoversVisible(bool visible)
        {
            foreach (StaticMover staticMover in staticMovers)
            {
